...
function processDirectory(baseDir) {
    ...
    if (!fs.existsSync(imagesDir)) {
        console.log(`No images dir in ${baseDir}`);
        return;
    }
    ...
    for (const filename of files) {
        ...
        if (newName !== filename) {
            ...
            console.log(`Renamed: '${filename}' -> '${newName}' in ${baseDir}`);
        }
    }
    
    console.log(`Total renames in ${baseDir}: ${Object.keys(renames).length}`);
